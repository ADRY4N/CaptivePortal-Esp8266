...
void setup() {
  Serial.begin(115200);
  Serial.println("\nIniciando Portal Cativo (versão com Correção de Notificação)...");
  ...
  // --- CORREÇÃO PARA NOTIFICAÇÃO NO CELULAR ---
  // Define o IP do portal como um IP conhecido de verificação de conectividade (Google)
  IPAddress apIP(172, 217, 28, 1);
  
  WiFi.mode(WIFI_AP);
  // Configura a rede do Access Point com o IP especial
  WiFi.softAPConfig(apIP, apIP, IPAddress(255, 255, 255, 0));
  // Inicia o Access Point com o SSID e senha configurados
  WiFi.softAP(config.ssid, config.password, config.channel);
  
  Serial.print("SSID: "); Serial.println(config.ssid);
  Serial.print("IP do Portal: http://"); Serial.println(WiFi.softAPIP());
  Serial.print("Tema Ativo: "); Serial.println(config.theme);

  // Inicia o servidor DNS para redirecionar todo o tráfego para o nosso IP
  ...
  server.onNotFound([](AsyncWebServerRequest *request) {
    ...
    Serial.printf("Captive portal request from host: %s, URL: %s. Serving theme page.\n", request->host().c_str(), request->url().c_str());
